...
def signup(request):
	...
	if request.method == 'POST':
		...
		number_check = User.objects.filter(username=number).exists()  ### this is already use mobile number register and this can check mobile number
		email_check = User.objects.filter(email=email).exists()### this is already use email register and this can check email
		...
		if len(number) != 10:
			...
			user = User.objects.create_user(username=number,email=email,password=cpassword)  ## create_user is store date in database
